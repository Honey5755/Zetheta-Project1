/**
 * E2E #2 (P0) — Home loan happy path: includes the (always-on) co-applicant
 * step and property documents (Spec E1).
 */
describe('Home loan happy path', () => {
  it('completes and submits a home loan with a co-applicant', () => {
    cy.fixture('valid-home-loan.json').then((d) => {
      cy.startApplication();

      cy.fillStep1(d.step1);
      cy.clickContinue();
      cy.fillStep2(d.step2);
      cy.clickContinue();
      cy.fillStep3(d.step3);
      cy.clickContinue();
      cy.fillStep4(d.step4);
      cy.clickContinue();
      cy.fillStep5Salaried(d.step5);
      cy.clickContinue();

      // Home loans always require a co-applicant.
      cy.expectStep('Co-Applicant');
      cy.fillCoApplicant(d.coApplicant);
      cy.clickContinue();

      cy.expectStep('Documents');
      cy.uploadDoc('Aadhaar card (front & back)', 'sample.pdf');
      cy.uploadDoc('Bank statements (last 6 months)', 'sample.pdf');
      cy.uploadDoc('Passport-size photograph', 'photo.png');
      cy.uploadDoc('Salary slips (last 3 months)', 'sample.pdf');
      cy.uploadDoc('Property documents', 'sample.pdf');
      cy.drawSignature();
      cy.wait(500);
      cy.clickContinue();

      cy.expectStep('Review');
      cy.checkConsents();
      cy.contains('button', 'Submit application').click();

      cy.contains('Application submitted').should('be.visible');
    });
  });
});
